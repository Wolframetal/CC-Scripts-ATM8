...
local checkList = parseBook(findCheckList().nbt.pages)
local tableShortageMaterials = {name = {}, count = {}}
local countAll = 0
...
for _, item in pairs(checkList.id) do -- compares items in checklist with RS
   repeat
      local answer = "n"
      itemRS = bridge.getItem({name = item})
      if itemRS.amount < checkList.n[_] then
         print("Out of [" .. item .. "], check your supplies! Need for construction: x" 
         .. checkList.n[_] - itemRS.amount .. "\n")   
   ...
   if itemRS.amount < checkList.n[_] then
      ...
      table.insert(tableShortageMaterials.count, tonumber(checkList.n[_] - itemRS.amount))
      sleep(1)
   else
...
itemRS = bridge.getItem({name = "minecraft:gunpowder"})
countAll = math.ceil(countAll / 400)
if itemRS.count < countAll / 400 then
